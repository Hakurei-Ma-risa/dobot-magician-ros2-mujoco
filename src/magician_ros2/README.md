...
:warning: The Dobot port defaults to **/dev/ttyUSB0**. Override it with the
`DOBOT_PORT` environment variable; a persistent `/dev/serial/by-id/...` path is
recommended for hardware deployments.

<a name="running"></a>
## System startup :robot:
1. Connect Dobot Magician with a USB cable to the computer and then turn it on.
2. Select the persistent serial device, for example `export DOBOT_PORT=/dev/serial/by-id/usb-1a86_USB_Single_Serial_598B002473-if00`.
3. Set the MAGICIAN_TOOL environment variable describing the robot's configuration `export MAGICIAN_TOOL=<tool_type>` (allowed values are: _none, pen, suction_cup, gripper, extended_gripper_).
4. From inside of the **magician_ros2_control_system_ws** directory, run `. install/setup.bash` to source your workspace.
5. Launch entire control stack with `ros2 launch dobot_bringup dobot_magician_control_system.launch.py`.
