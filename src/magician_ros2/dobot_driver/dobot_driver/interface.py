...
class Interface:
    def __init__(self, port):
        ...
        lock_id = hashlib.sha256(os.path.realpath(port).encode("utf-8")).hexdigest()[:16]
        self._process_lock_fd = os.open(
            f"/tmp/dobot-magician-{lock_id}.lock",
        ...
        try:
            ...
            print(f"[WARN] Dobot Magician is not connected: {exc}")
            sys.exit(1)

    @contextmanager
    def _transaction(self):
        """Serialize access across threads and ROS processes sharing one UART."""
    ...
    def send(self, message):
        with self._transaction():
            # Write-only protocol commands can leave an acknowledgement behind.
            # Discard it before beginning the next request/response transaction.
    ...
    def send_only(self, message):
        with self._transaction():
            ...
            self.serial.write(message.package())
            self.serial.flush()
            # The caller intentionally does not consume a response. Give the
            # controller time to emit any acknowledgement, then discard it
            # while the inter-process transaction lock is still held.
            time.sleep(0.01)
            self.serial.reset_input_buffer()
